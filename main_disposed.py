...
from train import (
    ...
    train_dl_har, train_dl_wisdm,
)
from sklearn.metrics import accuracy_score
from test import evaluate_har, evaluate_wisdm, plot_confusion_matrix
...
def run_har_pipeline():
    ...
    # ---- 深度学习模型 ----
    har_dl_results, _ = train_dl_har(
        scaler.transform(X_train), y_train, X_test_s, y_test,
        train_inertial, test_inertial, activities,
        epochs=80,
    )
    for dl_name, dl_res in har_dl_results.items():
        har_acc[dl_name] = dl_res['acc']
        # DL 混淆矩阵
        cmap = {'DL-MLP': 'Blues', 'DL-CNN': 'Greens', 'DL-CNNLSTM': 'Purples'}.get(dl_name, 'default')
        plot_confusion_matrix(
            y_test, dl_res['y_pred'], activities,
            f'HAR - {dl_name} Confusion Matrix',
            os.path.join(OUTPUT_DIR, f'har_cm_{dl_name.lower().replace("-", "_")}.png'),
            cmap_name=cmap,
        )

    return activities, har_acc, rf, feat_names, svm, knn, scaler, X_test_s, y_test, X_train, y_train, har_dl_results
...
def run_wisdm_pipeline():
    ...
    scaler, knn, svm, rf, X_test, y_test, seg_train, seg_test, X_train_w, y_train_w = prepare_wisdm(DATA_DIR)
    ...
    # ---- 深度学习模型 ----
    wisdm_dl_results = train_dl_wisdm(
        scaler.transform(X_train_w), y_train_w, X_test_s, y_test,
        seg_train, seg_test,
        epochs=80,
    )
    for dl_name, dl_res in wisdm_dl_results.items():
        wisdm_acc[dl_name] = dl_res['acc']
        # DL 混淆矩阵
        labels_sorted = sorted(np.unique(y_test))
        cmap = {'DL-MLP': 'Blues', 'DL-CNN': 'Greens', 'DL-CNNLSTM': 'Purples'}.get(dl_name, 'default')
        plot_confusion_matrix(
            y_test, dl_res['y_pred'], labels_sorted,
            f'WISDM - {dl_name} Confusion Matrix',
            os.path.join(OUTPUT_DIR, f'wisdm_cm_{dl_name.lower().replace("-", "_")}.png'),
            cmap_name=cmap,
        )

    return wisdm_acc, X_major, y_major, wisdm_feat_names, svm, knn, rf, scaler, X_test_s, y_test, wisdm_dl_results
...
def main():
    ...
    (activities, har_acc, rf_model, feat_names,
     ...
     X_train_har, y_train_har, har_dl_results) = run_har_pipeline()

    (wisdm_acc, X_major, y_major, wisdm_feat_names,
     svm_w, knn_w, rf_w, scaler_w, X_test_s_w, y_test_w,
     wisdm_dl_results) = run_wisdm_pipeline()
